...
class exceptieCapacitate : public exception {
...
class exceptieBinarySearch : public exception {
	virtual const char* what() const throw()
	{
		return " Binary search: Vectorul nu e sortat!";
	}
};

class exceptieNotFind : public exception
{
	virtual const char* what() const throw()
	{
...
int main()
{
	try
	{
		...
		cout << "initial: ";
		a.Print();
		a.Sort(&comparatorInt);
		cout << endl << "Sortat: ";
		a.Print();
		cout << "\n5 se gaseste pe pozitia: " << a.BinarySearch(5, &comparatorInt) << endl;
	}
	catch (exception& e)
	{
		cout << "Exceptie: " << e.what() << endl;
